...
function pickPreferredFemaleVoice() {
    ...
    const femaleVoice = voices.find((voice) => {
        ...
        return lang.startsWith("en") && preferredPatterns.some((pattern) => name.includes(pattern));
    });

    if (femaleVoice) return femaleVoice;

    // Fallback to any English voice if a female-tagged name is unavailable.
    return voices.find((voice) => (voice.lang || "").toLowerCase().startsWith("en")) || voices[0];
}

function ensurePreferredVoiceLoaded() {
    preferredFemaleVoice = pickPreferredFemaleVoice();
}

if ("speechSynthesis" in window) {
    ensurePreferredVoiceLoaded();
    window.speechSynthesis.onvoiceschanged = ensurePreferredVoiceLoaded;
}
...
function speakText(text, onComplete) {
    ...
    if (preferredFemaleVoice) {
        ...
        utterance.lang = preferredFemaleVoice.lang || "en-US";
    }
...
async function submitAnswer() {
    ...
    try {
        ...
        const contentType = response.headers.get("content-type");
        const isCompleteHeader = response.headers.get('X-Interview-Complete') === 'true';
        ...
        if (contentType && contentType.includes("text/plain")) {
            handleAudioStream(response, () => {
                recordedBlob = null;
                recordingChunks = [];
                const isComplete = isCompleteHeader;
            ...
            recordedBlob = null;
            recordingChunks = [];
            const isComplete = isCompleteHeader || data.interview_completed === true;

            if (data.question_number) {
                updateQuestionNumber(data.question_number);
            }
...
async function getFeedback() {
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            getFeedbackBtn.textContent = data.error || "Error - Retry";
            getFeedbackBtn.disabled = false;
        }
    } catch (error) {
